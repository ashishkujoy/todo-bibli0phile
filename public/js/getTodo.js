...
const handleDelete = function(res) {
  if(res.status === 202) window.location = '/viewTodo.html';
};

const deleteTodo = function () {
    let todoId = document.querySelector("#title p").id;
    sendAjaxRequest('POST', '/delete', function () {
        handleDelete(this);
    }, `todoId=${todoId}`);
};
